...
| D3 | Rebuild the model in Power BI | Done | Three-page report (`General Overview`, `Region & Segment Overview`, `Printing Overview`) in `dashboards/power_bi_report.pbix`. KPI cards, a TTM line, a bookmark-driven Region/Segment toggle, DAX measures in active use. Verified by reading the finished file's PBIR JSON: bookmark visibility states are exact complements, visual field bindings and filters confirmed, numbers hand-checked against the CSVs. Full build and bug log in `dashboard_notes.md` §6. Issue #16. |
| D4 | Draft and review DAX measures | Done | `Net Revenue`, `Printing Revenue`, `Regional Revenue`, `Operating Margin` all drafted in `docs/dax_measures.md` and confirmed in active use in the finished report. `Region % of Total` and `Segment % of Total` (the `CALCULATE`/`DIVIDE`/filter-context pair) were built and debugged at length but never worked, root cause not identified within the time available; recorded honestly as an outcome note rather than removed. Both percentage visuals fall back to the already-verified `dim_date` columns instead. |
