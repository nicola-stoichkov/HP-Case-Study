...
Extended 2026-08-31 with two more share-of-parent rows: Commercial Printing and Consumer Printing as a percentage of Printing, same `matrix`-reference pattern as the original Supplies row. Inserting them shifted every row below by two, and Google Sheets auto-adjusted every downstream formula reference correctly, confirmed by re-checking the growth decomposition and all twelve implied-profit rows after the shift, all still exact.

## 6. Two dimension tables, `dim_business_unit` and `dim_date` (D1)

Built to support the Looker Studio and Power BI dashboards: `dim_business_unit` carries an `is_rollup` flag and a `parent_unit` column, encoding trap 1 (leaf vs. rollup) directly into the data model rather than relying on every chart to filter it correctly. `dim_date` carries an explicit `sort_order` so no BI tool needs to be told separately how to sort fiscal quarters chronologically, plus three metrics referenced from `derived` (TTM, both share-of-parent percentages) so a row-per-quarter table exists for tools that need one.

Two cells in `dim_date` (`F2`, `I2`) briefly held a `TRANSPOSE` formula sized for an entire row placed in a single cell, colliding with values already below it, which broke Looker Studio's connection to the whole tab. Fixed by pasting both as static values instead, matching every other cell in those two columns, which were already static rather than live. Full incident writeup in `docs/dashboard_notes.md`.
